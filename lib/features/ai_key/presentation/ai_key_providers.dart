...
import 'package:fbr_taxvault/features/ai_key/data/user_ai_key_repository_impl.dart';
...
final userAiKeyRepositoryProvider = Provider<UserAiKeyRepository>((ref) {
  return UserAiKeyRepositoryImpl(ref.watch(supabaseClientProvider));
});

/// The signed-in user's own key status within [organizationId], separate
/// from the org-level [geminiKeyStatusProvider] an owner/admin manages.
final myGeminiKeyStatusProvider =
    FutureProvider.family<GeminiKeyStatus, String>((ref, organizationId) async {
      final result = await ref
          .watch(userAiKeyRepositoryProvider)
          .getStatus(organizationId);
      return result.fold((status) => status, (failure) => throw failure);
    });
