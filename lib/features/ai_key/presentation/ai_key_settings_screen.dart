...
/// Lets an org owner/admin add or replace the organization's own Gemini API
/// key — each org brings its own key rather than sharing one across every
/// TaxVault customer, so quota and billing stay scoped per org. Scanning is
/// blocked (see ScannerScreen) until a key is configured here.
class AiKeySettingsScreen extends ConsumerStatefulWidget {
  const AiKeySettingsScreen({super.key});

  @override
  ConsumerState<AiKeySettingsScreen> createState() =>
      _AiKeySettingsScreenState();
}

class _AiKeySettingsScreenState extends ConsumerState<AiKeySettingsScreen> {
  ...
  Future<void> _save(String organizationId) async {
    ...
    result.fold(
      (_) {
        ref.invalidate(geminiKeyStatusProvider(organizationId));
          ...
          : SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Each organization uses its own Gemini API key for document scanning — this keeps usage quota and billing separate per organization.',
...
class _StatusCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final (color, icon) = !status.hasKey
        ? (theme.colorScheme.onSurfaceVariant, Icons.key_off_outlined)
        : status.isValid
        ? (semantic.success, Icons.check_circle_rounded)
